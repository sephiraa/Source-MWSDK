...
// Purpose: 
//
// $NoKeywords: $FixedByTheMaster974, also be sure to give credit to Saul Rennison!
//
...
#include "tutorials/worldlight.h"
...
// Additions.
// ----------
		Vector					m_ShadowDir;		// Shadow direction.
...
private:
	...
	const Vector& GetShadowDirection(ClientShadowHandle_t shadowHandle) const; // Addition.
...
private:
	...
	bool m_bShadowFromWorldLights; // Addition.
	...
	m_bDepthTextureActive( false )
{
	...
	m_bThreaded = false;
	m_bShadowFromWorldLights = r_worldlight_castshadows.GetBool(); // Addition.
}
...
void CClientShadowMgr::UpdateShadow( ClientShadowHandle_t handle, bool force )
{
	...
	if (force || (origin != shadow.m_LastOrigin) || (angles != shadow.m_LastAngles) || shadow.m_LightPosLerp < 1.0f) // Added to if statement.
	{
